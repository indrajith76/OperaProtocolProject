...
const LaunchToken = () => {
  ...
  const { unstakeEth,lendEth,txPending,userData,userBalance,connectWallet, addressOfUser } = useOutletContext();
    ...
    async function setTaxesZero(){
      document.getElementById("marketingBuyInput").value = 0
      document.getElementById("marketingSellInput").value=0
      document.getElementById("devBuyInput").value=0
      document.getElementById("devSellInput").value=0
      document.getElementById("liqBuyInput").value=0
      document.getElementById("liqSellInput").value=0
    }
  async function handleDeployClick(){
    let nameInput = document.getElementById("nameInput").value
    let symbolInput = document.getElementById("symbolInput").value
    let swapInput = document.getElementById("swapInput").value
    let maxWalletInput = document.getElementById("maxWalletInput").value
    let supplyInput = document.getElementById("supplyInput").value
    let marketingBuyInput = document.getElementById("marketingBuyInput").value
    let marketingSellInput = document.getElementById("marketingSellInput").value
    let devBuyInput = document.getElementById("devBuyInput").value
    let devSellInput = document.getElementById("devSellInput").value
    let liqBuyInput = document.getElementById("liqBuyInput").value
    let liqSellInput = document.getElementById("liqSellInput").value
    ...
    let tempBool = false
    if(!supplyInput || ! swapInput || !maxWalletInput){
      return
    }
    if(borrowedEth===0){
      tempBool = true
    ...
    if(liqSellInput + devSellInput + marketingSellInput > 96){
      setSellTaxError(true)
      tempBool = true
    } else {
      setSellTaxError(false)
    }
    if(liqBuyInput + devBuyInput + marketingBuyInput > 96){
      setBuyTaxError(true)
      tempBool = true
    } else {
      setBuyTaxError(false)
    }
    if(initialSwapThreshold < 0.5 || initialSwapThreshold > 5 ){
      setSwapError(true)
      tempBool = true
    } else {
      setSwapError(false)
    }
    if(initialMaxWallet < 0.5  ){
    ...
    setModalOn({stringList,addressList,numbersList,borrowedEth})      
    }
        ...
        <div className="mt-0 grid grid-cols-1 lg:grid-cols-2 gap-20">
          <div>
            ...
            <p style={{color:maxWalletError ? "red":""}} className="text-[12px] ml-[10px] mb-[13px]">
              Max Wallet must be 0.5% or greater
            </p>
            <div style={{display:"flex"}}>
            <input onChange={setTaxesZero} type="checkbox">

            </input>
            <p style={{marginLeft:"20px"}}>0% TAXES</p>
            </div>
